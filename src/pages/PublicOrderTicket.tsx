...
export default function PublicOrderTicket() {
  const { storeId: routeStoreId, orderId } = useParams();
  const storeId = getStoreIdFromHostname(window.location.hostname) || routeStoreId;
  ...
  useEffect(() => {
    async function fetchData() {
      try {
        ...
        // Fetch store data
        const storeRes = await fetch(getApiUrl(`/api/stores/${storeId}`));
        ...
        // Fetch order data
        const orderRes = await fetch(getApiUrl(`/api/stores/${storeId}/orders/${orderId}`));
  ...
  if (error || !order || !settings) {
    return (
      <div className="min-h-screen flex flex-col items-center justify-center bg-white p-4">
        ...
        <button onClick={() => window.location.href = getStoreIdFromHostname(window.location.hostname) ? '/' : `/s/${storeId}`} className="mt-6 px-6 py-2 bg-gray-900 text-white rounded-xl font-bold transition-all active:scale-95">Voltar para a Loja</button>
      </div>
    );
  }
    ...
    <div className="min-h-screen bg-gray-100 py-8 px-4 sm:px-6">
      <div className="max-w-md mx-auto">
        <div className="mb-4 flex justify-between items-center no-print">
          <button 
            onClick={() => window.location.href = getStoreIdFromHostname(window.location.hostname) ? '/' : `/s/${storeId}`}
