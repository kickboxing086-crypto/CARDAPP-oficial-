...
export default function CustomerView({ overrideStoreId }: { overrideStoreId?: string }) {
  ...
  const storeId = overrideStoreId || getStoreIdFromHostname(window.location.hostname) || routeStoreId;
  ...
  const formatPhone = (value: string) => {
    ...
    // Auto-strip leading 55 if they typed or pasted with country code DDI (e.g., 5584986113980)
  ...
  const [cep, setCep] = useState(() => localStorage.getItem('cust_cep') || '');
  const [street, setStreet] = useState(() => localStorage.getItem('cust_street') || '');
  const [number, setNumber] = useState(() => localStorage.getItem('cust_number') || '');
  const [neighborhood, setNeighborhood] = useState(() => localStorage.getItem('cust_neighborhood') || '');
  const [city, setCity] = useState(() => localStorage.getItem('cust_city') || '');
  const [zone, setZone] = useState(() => localStorage.getItem('cust_zone') || '');
  const [reference, setReference] = useState(() => localStorage.getItem('cust_reference') || '');
  const [isFetchingCep, setIsFetchingCep] = useState(false);

  useEffect(() => {
    localStorage.setItem('cust_cep', cep);
  }, [cep]);

  useEffect(() => {
    localStorage.setItem('cust_street', street);
  }, [street]);

  useEffect(() => {
    localStorage.setItem('cust_number', number);
  }, [number]);

  useEffect(() => {
    localStorage.setItem('cust_neighborhood', neighborhood);
  }, [neighborhood]);
  ...
  useEffect(() => {
    localStorage.setItem('cust_zone', zone);
  }, [zone]);

  useEffect(() => {
    localStorage.setItem('cust_reference', reference);
  }, [reference]);
  ...
  useEffect(() => {
    if ((isScheduled || !isCurrentlyOpen) && scheduledDate && storeId && settings?.allowScheduling) {
      fetch(getApiUrl(`/api/stores/${storeId}/booked-slots?date=${scheduledDate}`))
        .then(res => res.json())
  ...
  useEffect(() => {
    ...
    const fetchLatestStatuses = async () => {
      ...
      const newOrders = await Promise.all(customerOrders.map(async (order) => {
        ...
        try {
          const r = await fetch(getApiUrl(`/api/stores/${initialStoreId}/orders/${order.id}`));
          if (r.ok) {
  ...
  useEffect(() => {
    ...
    // 2. Detect In-App Browser (Instagram, Facebook, WhatsApp, etc.)
    const ua = navigator.userAgent || '';
    const isInApp = ua.includes('Instagram') || ua.includes('FBAV') || ua.includes('FBAN') || ua.includes('WhatsApp');
  ...
  const fetchProducts = async () => {
    ...
    try {
      const res = await fetch(getApiUrl(`/api/stores/${storeId}/products?t=${Date.now()}`), {
  ...
  const isTrialExpired = settings?.plan === 'FREE_LIFETIME' ? false : (settings?.createdAt ? (new Date().getTime() - new Date(settings.createdAt).getTime()) > 7 * 24 * 60 * 60 * 1000 : false);
  ...
  useEffect(() => {
    const loadSettings = async () => {
      try {
        const res = await fetch(getApiUrl(`/api/stores/${storeId}/settings?t=${Date.now()}`), {
          headers: { 'Cache-Control': 'no-cache, no-store' }
        });
        ...
        setStoreError(e.message || 'Erro ao carregar configurações da loja');
      }
    };
    ...
    // Poll for real-time product & stock updates every 10 seconds
  ...
  const handleCheckout = async (e?: React.FormEvent) => {
    ...
    try {
      res = await fetch(getApiUrl(`/api/stores/${storeId}/orders`), {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify(orderData)
      });
    } catch (fetchErr) {
      console.error('API error submitting order:', fetchErr);
      setIsSubmitting(false);
      alert('Erro de conexão ao enviar pedido. Tente novamente em alguns instantes.');
      return;
    }
      ...
      if (!data) {
        ...
        data = {
          ...
          items: cart.map(c => ({ 
            ...
            addons: c.addons,
            flavors: c.flavors,
            observation: c.productObservation
          })),
          totalPrice: cartTotal + selectedCityFee,
          scheduledDate: isScheduled ? scheduledDate : undefined,
          scheduledTime: isScheduled ? scheduledTime : undefined,
  ...
  const generateWhatsAppUrl = (orderToUse?: Order): string => {
    ...
    const rawPhone = settings?.whatsappNumber || '5511999999999';
    const digitsOnly = rawPhone.replace(/\D/g, '');
    const adminPhone = digitsOnly.startsWith('55') ? digitsOnly : `55${digitsOnly}`;
    ...
    order.items.forEach(item => {
      ...
      if ((item as any).observation) {
        text += `   _Obs: ${(item as any).observation}_\n`;
      }
    });
  ...
  if (storeError) {
    return (
      <div className="min-h-screen flex items-center justify-center bg-gray-50 flex-col gap-4">
        ...
        {storeError && typeof storeError === 'string' && storeError !== '' && <p className="text-red-500 text-xs font-mono max-w-sm text-center bg-red-50 p-2 rounded">{storeError}</p>}
        <Link to="/admin/login" className="px-6 py-2 bg-gray-800 hover:bg-gray-700 text-white rounded-lg mt-4 font-medium transition-colors">Sou Lojista</Link>
      </div>
    );
  }
  ...
  if (!settings) {
    return (
      <div className="min-h-screen flex flex-col items-center justify-center bg-white text-gray-800 relative overflow-hidden">
        ...
        >
          <div className="w-20 h-20 bg-gray-50 rounded-2xl flex items-center justify-center mb-6 border border-gray-100 shadow-xl">
          ...
          <p className="text-sm font-medium text-gray-500">Preparando a loja para você</p>
        </motion.div>
        
        {/* Decorative background elements */}
        <div className="absolute top-1/4 left-1/4 w-64 h-64 bg-emerald-500/10 blur-[100px] rounded-full mix-blend-multiply" />
        <div className="absolute bottom-1/4 right-1/4 w-64 h-64 bg-blue-500/10 blur-[100px] rounded-full mix-blend-multiply" />
      </div>
    );
  }
  ...
  return (
    <div className="pb-24 custom-font-store" style={{ '--theme-color': settings.primaryColor } as React.CSSProperties}>
      ...
      <div className="bg-white pt-8 pb-6 px-5 border-b border-gray-150 relative z-10 shadow-[0_4px_20px_rgba(0,0,0,0.03)]">
        <div className="max-w-3xl mx-auto flex flex-col sm:flex-row items-center sm:items-start gap-6">
          
          {/* Left: Logo (Instagram Style Avatar with Story Ring) */}
          <div className="relative shrink-0">
            <div className={`w-28 h-28 sm:w-32 sm:h-32 rounded-full p-[3px] bg-gradient-to-tr ${isCurrentlyOpen ? 'from-emerald-400 via-emerald-500 to-emerald-600' : 'from-gray-300 to-gray-400'} shadow-sm`}>
              <div className="w-full h-full p-[3px] bg-white rounded-full">
                {settings.logo ? (
                  <img src={settings.logo} className="w-full h-full object-cover rounded-full border border-gray-100" alt="Logo" referrerPolicy="no-referrer" />
                ) : (
                  <div className="w-full h-full text-white rounded-full flex items-center justify-center text-4xl font-black uppercase tracking-widest shadow-inner" style={{ backgroundColor: settings.primaryColor }}>
            ...
            {isCurrentlyOpen && (
              <div className="absolute bottom-2 right-2 w-7 h-7 bg-emerald-500 rounded-full border-[3px] border-white flex items-center justify-center shadow-sm" title="Atendimento Ativo">
                <div className="w-2.5 h-2.5 bg-white rounded-full animate-ping" />
              </div>
            )}
          </div>

          {/* Right: Info (Instagram Style Bio) */}
          <div className="flex-1 text-center sm:text-left space-y-3 w-full">
            
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
              <h1 className="text-2xl sm:text-3xl font-black text-gray-900 tracking-tight flex items-center justify-center sm:justify-start gap-1.5">
              ...
              <div>
                <span className={`inline-flex px-3 py-1.5 border ${isCurrentlyOpen ? 'bg-emerald-50 text-emerald-800 border-emerald-150' : 'bg-rose-50 text-rose-800 border-rose-150'} text-[10px] font-black uppercase tracking-widest rounded-full`}>
            ...
            <div className="text-sm text-gray-800 space-y-1.5">
               {settings.ceoName && (
                 <div className="font-bold flex items-center justify-center sm:justify-start gap-1.5 text-base">
                   {settings.ceoName} 
                   <span className="text-[10px] font-bold text-gray-500 bg-gray-100 px-2 py-0.5 rounded-md uppercase tracking-wider">CEO</span>
                 </div>
               )}
               {settings.storeTagline && (
                 <div className="text-gray-500 font-medium text-sm">{settings.storeTagline}</div>
               )}
               {settings.description && (
                 <div className="whitespace-pre-wrap mt-2 text-sm text-gray-700 leading-relaxed max-w-2xl mx-auto sm:mx-0">{settings.description}</div>
               )}
            </div>

            {(settings.instagramUrl || settings.websiteUrl) && (
              <div className="flex flex-wrap items-center justify-center sm:justify-start gap-2 pt-2">
                {settings.instagramUrl && (
                  <a href={settings.instagramUrl} target="_blank" rel="noopener noreferrer" className="text-blue-700 font-bold flex items-center gap-1.5 hover:underline text-xs bg-blue-50/50 hover:bg-blue-50 px-3 py-2 rounded-xl transition-colors">
                    <Instagram className="w-4 h-4" />
                    {settings.instagramUrl.replace('https://instagram.com/', '@').replace('https://www.instagram.com/', '@').replace(/\/$/, '')}
                  </a>
                )}
                {settings.websiteUrl && (
                  <a href={settings.websiteUrl} target="_blank" rel="noopener noreferrer" className="text-blue-700 font-bold flex items-center gap-1.5 hover:underline text-xs bg-blue-50/50 hover:bg-blue-50 px-3 py-2 rounded-xl transition-colors">
                    <Globe className="w-4 h-4" />
                    {settings.websiteUrl.replace(/^https?:\/\/(www\.)?/, '').replace(/\/$/, '')}
                  </a>
                )}
              </div>
            )}
        ...
        {/* Action Buttons (Like Instagram Edit Profile / Share) */}
        <div className="max-w-3xl mx-auto mt-6 flex gap-2">
          <div 
             className="flex-1 flex flex-col sm:flex-row items-center justify-center gap-1 sm:gap-2 p-2.5 sm:p-3 rounded-2xl bg-gray-100 hover:bg-gray-200 transition-colors cursor-pointer text-[11px] sm:text-xs font-bold text-gray-800 border border-transparent hover:border-gray-300"
          ...
          {settings.deliveryTime ? (
            <div className="flex-1 flex flex-col sm:flex-row items-center justify-center gap-1 sm:gap-2 p-2.5 sm:p-3 rounded-2xl bg-[#EEF2FF] border border-indigo-50">
               <Truck className="w-4 h-4 text-indigo-500 shrink-0" />
               <span className="text-[11px] sm:text-xs font-extrabold text-indigo-800 text-center">Envio: {settings.deliveryTime}</span>
            </div>
          ) : (
            <div className="flex-1 flex flex-col sm:flex-row items-center justify-center gap-1 sm:gap-2 p-2.5 sm:p-3 rounded-2xl bg-gray-100 hover:bg-gray-200 transition-colors cursor-pointer text-[11px] sm:text-xs font-bold text-gray-800 border border-transparent hover:border-gray-300">
          ...
          >
             <Share className="w-4 h-4 text-gray-600 shrink-0" />
             Compartilhar
          </div>
        </div>
      </div>
      ...
      {!isCurrentlyOpen && settings?.allowScheduling && (
        <div className="max-w-3xl mx-auto px-5 mt-4">
          <div className="animate-in fade-in slide-in-from-top-4 duration-700 flex flex-col gap-3">
            <div 
                        ...
                        return (
                          ...
                          >
                            <div className="relative aspect-[4/5] bg-gray-50 overflow-hidden">
                              ...
                              {product.promoQuantity > 0 && !isUnavailable && (
                                <motion.div 
                                  initial={{ x: -20, opacity: 0 }}
                                  animate={{ x: 0, opacity: 1 }}
                          ...
                          <div className="p-4 flex flex-col items-center text-center flex-grow">
                            {settings?.productOrder === 'price-first' ? (
                              <>
                                <div className="font-bold whitespace-nowrap flex flex-col items-center gap-0.5 text-sm sm:text-base mb-1">
                                  {product.promotion && product.promoPrice && product.promoQuantity && product.promoQuantity > 1 ? (
                                    <div className="flex flex-col items-center bg-amber-500/10 px-3 py-1 rounded-xl border border-amber-500/30 text-center">
                                      <span className="text-[10px] text-amber-400 font-black uppercase tracking-tight flex items-center gap-1">
                                        🔥 LEVE {product.promoQuantity} POR R$ {product.promoPrice.toFixed(2)}
                                      </span>
                                      <span className="text-[9px] text-neutral-400 font-bold">
                                        (R$ {(product.promoPrice / product.promoQuantity).toFixed(2)}/un)
                                      </span>
                                    </div>
                                  ) : product.promotion && product.promoPrice && product.promoPrice > 0 ? (
                                    <div className="flex flex-col items-center bg-amber-500/10 px-3 py-1 rounded-xl border border-amber-500/30 text-center">
                                      <span className="text-[10px] text-neutral-400 font-bold leading-none flex gap-1 items-center mb-0.5">
                                        De: <span className="line-through text-neutral-400">R$ {product.price.toFixed(2)}</span>
                                      </span>
                                      <span className="flex items-baseline gap-1 font-black text-amber-400 text-sm sm:text-base">
                                        Por: R$ {product.promoPrice.toFixed(2)}
                                        <span className="text-[10px] font-bold text-amber-300">/{product.unit || 'UN'}</span>
                                      </span>
                                    </div>
                                  ) : (
                                    <span className="flex items-baseline gap-0.5 font-black text-amber-400 text-sm sm:text-base">
                                      R$ {product.price ? product.price.toFixed(2) : '0.00'}
                                      <span className="text-[10px] sm:text-xs font-medium text-neutral-400">/{product.unit || 'UN'}</span>
                                    </span>
                                  )}
                                </div>
                                <h4 className="font-bold text-white leading-tight mb-1 text-center">{product.name}</h4>
                              </>
                            ) : (
                              <>
                                <h4 className="font-bold text-white leading-tight mb-1 text-center">{product.name}</h4>
                          ...
                          {settings?.productOrder === 'price-first' ? (
                            <>
                              <div className="flex flex-col mb-1 block">
                                {product.promotion && product.promoPrice && product.promoQuantity && product.promoQuantity > 1 ? (
                                  <div className="flex flex-col bg-amber-50 px-2 py-1 rounded-lg border border-amber-200/80 w-fit mb-0.5">
                                    <span className="text-[10px] text-amber-700 font-extrabold leading-none flex gap-1 items-center">
                                      🔥 LEVE {product.promoQuantity} POR R$ {product.promoPrice.toFixed(2)}
                                    </span>
                                    <span className="text-[9px] text-amber-600/80 font-semibold mt-0.5">
                                      (R$ {(product.promoPrice / product.promoQuantity).toFixed(2)} cada un)
                                    </span>
                                  </div>
      ...
      {/* Footer */}
      <footer className="mt-20 py-12 bg-gray-50 text-center border-t border-gray-200 pb-32 text-gray-650">
        <div className="flex flex-col items-center justify-center mb-6">
          <div className="flex items-center gap-2 bg-white border border-gray-200 rounded-full px-5 py-2 shadow-sm">
            <FoodTrayIcon className="w-4 h-4 text-[--theme-color] shrink-0" />
            <span className="text-sm font-black tracking-widest text-[--theme-color] uppercase">CARDAPP</span>
            <span className="w-2 h-2 rounded-full bg-[--theme-color] animate-pulse shadow-[0_0_8px_var(--theme-color)]" />
          </div>
        </div>

        <div className="flex flex-col md:flex-row items-center justify-center gap-4 md:gap-8 mb-6 mt-4">
          <button onClick={() => setDocModal('terms')} className="text-gray-500 hover:text-[--theme-color] font-bold text-xs transition-colors uppercase tracking-widest cursor-pointer">
            Termos de Uso
          </button>
          <button onClick={() => setDocModal('privacy')} className="text-gray-500 hover:text-[--theme-color] font-bold text-xs transition-colors uppercase tracking-widest cursor-pointer">
            Política de Privacidade
          </button>
        </div>
        <p className="text-[10px] text-gray-400 font-bold uppercase tracking-widest px-4">
          {new Date().getFullYear()} {settings.storeNameFirst || settings.storeName}. Todos os direitos reservados.
        </p>
        <div className="mt-8 flex flex-col items-center gap-2">
          <Link to="/admin/login" className="text-[9px] text-gray-400 hover:text-[--theme-color] transition-colors uppercase tracking-wider font-bold">
            Área do Administrador
          </Link>
          <span className="text-[9px] uppercase font-bold tracking-widest text-gray-400 mt-4">
            Desenvolvido por <span className="text-[--theme-color] font-extrabold">SFTECNOLOGIA</span>
          </span>
        </div>
      </footer>
            ...
            <div className="p-5 border-t border-gray-100 bg-white space-y-5">
              ...
              >
                ...
                <div className="flex flex-col items-end leading-none">
                  ...
                  {addonModalProduct.promoQuantity > 0 && ((addonModalProduct.flavors && addonModalProduct.flavors.length > 0) ? Object.values(flavorQuantities as Record<string, number>).reduce((a: number, b: number) => a + b, 0) : modalQuantity) >= addonModalProduct.promoQuantity && (
                  ...
                  <div className="bg-emerald-50 border border-emerald-100 p-4 rounded-2xl flex flex-col gap-2 animate-in slide-in-from-bottom-2 duration-300">
                    <div className="flex items-center gap-3">
                      ...
                      <div className="flex-1">
                        <p className="text-[11px] font-black uppercase text-emerald-700 tracking-tight leading-tight mb-0.5">Reserva para {formattedScheduleDate} Ativada</p>
          ...
          >
            ...
            <div className="flex items-center justify-between border-b border-gray-100 pb-3 mt-1">
              <div className="flex items-center gap-2">
                ...
                <h3 className="text-xs font-black text-gray-905 uppercase tracking-widest">Endereço Físico</h3>
              </div>
              <button 
                onClick={() => {
                  setShowLocationModal(false);
      ...
      <footer className="mt-12 py-12 border-t border-gray-100 flex flex-col items-center gap-6">
        <div className="flex flex-col items-center gap-2">
          <span className="text-[9px] font-bold uppercase tracking-[0.2em] text-gray-400">Tecnologia Oficial</span>
          <div className="flex items-center gap-2.5 px-4 py-2 bg-zinc-950 text-white rounded-full border border-zinc-800 shadow-md">
            <CardappLogo size="sm" />
            <div className="w-1.5 h-1.5 rounded-full bg-amber-400 animate-pulse ml-1" />
            <span className="text-[9px] font-black text-amber-400 uppercase tracking-widest">SISTEMA ATIVO</span>
          </div>
        </div>
        
        <div className="flex flex-wrap justify-center gap-6">
          <a href="https://www.instagram.com/cardapp.app?igsh=MTgyZnhxZXNrNnU1OQ==" target="_blank" rel="noreferrer" className="text-xs font-bold text-gray-400 hover:text-amber-600 transition-colors flex items-center gap-1">
            <Instagram className="w-3.5 h-3.5" /> Instagram
          </a>
          <Link to="/termos-e-privacidade" className="text-xs font-bold text-gray-400 hover:text-amber-600 transition-colors">Termos de Uso</Link>
          <Link to="/termos-e-privacidade" className="text-xs font-bold text-gray-400 hover:text-amber-600 transition-colors">Privacidade</Link>
          <Link to="/" className="text-xs font-bold text-gray-400 hover:text-amber-600 transition-colors">Criar minha Loja</Link>
        </div>
      ...
      <AnimatePresence>
        {showPwaAssistant && (
          <div 
            className="fixed inset-0 bg-black/60 backdrop-blur-xs flex items-center justify-center p-4 z-[130]"
            onClick={() => setShowPwaAssistant(false)}
          >
            <motion.div 
              initial={{ scale: 0.9, opacity: 0 }}
              animate={{ scale: 1, opacity: 1 }}
              exit={{ scale: 0.9, opacity: 0 }}
              transition={{ type: "spring", damping: 25, stiffness: 350 }}
              className="bg-white rounded-3xl max-w-sm w-full p-6 shadow-2xl border border-gray-100 flex flex-col gap-5 overflow-hidden relative text-left"
              onClick={(e) => e.stopPropagation()}
            >
              <div className="absolute top-0 inset-x-0 h-1.5" style={{ backgroundColor: settings?.primaryColor || '#10b981' }} />

              <div className="flex items-center justify-between border-b border-gray-100 pb-3 mt-1">
                <div className="flex items-center gap-2">
                  <Smartphone className="w-5 h-5 text-indigo-500" />
                  <h3 className="text-sm font-black text-gray-900 uppercase tracking-tight">Instalar Aplicativo</h3>
                </div>
                <button 
                  onClick={() => setShowPwaAssistant(false)}
                  className="p-1.5 px-3 bg-gray-50 hover:bg-gray-100 text-gray-500 font-bold text-xs rounded-xl transition-all cursor-pointer"
                >
                  Fechar
                </button>
              </div>
              ...
              {isInAppBrowser ? (
                // Scenario 1: Inside Instagram/WhatsApp Internal WebView
                <div className="space-y-4">
                  <div className="p-4 bg-amber-50 border border-amber-200 rounded-2xl space-y-2">
                    <div className="flex items-start gap-2.5 text-amber-800">
                      <AlertTriangle className="w-5 h-5 shrink-0 mt-0.5 text-amber-600 animate-pulse" />
                      <div>
                        <p className="text-xs font-black uppercase tracking-wider">Navegador Interno Ativo</p>
                        <p className="text-xs font-bold text-amber-700 leading-relaxed mt-1">
                          Você abriu este link de dentro de uma rede social (Instagram, WhatsApp ou Facebook). Esses navegadores limitam a instalação de aplicativos!
                        </p>
                      </div>
                    </div>
                  </div>
                  ...
                  <div className="space-y-3">
                    <p className="text-xs font-bold text-gray-700">Para instalar corretamente no seu celular, siga as instruções:</p>
                    <div className="space-y-2">
                      <div className="flex items-start gap-3">
                        <div className="w-5 h-5 rounded-full bg-slate-900 text-white text-[10px] font-black flex items-center justify-center shrink-0">1</div>
                        <p className="text-xs font-bold text-gray-600">Toque nos <span className="font-black text-slate-800">três pontinhos (...)</span> no topo superior direito da tela.</p>
                      </div>
                      <div className="flex items-start gap-3">
                        <div className="w-5 h-5 rounded-full bg-slate-900 text-white text-[10px] font-black flex items-center justify-center shrink-0">2</div>
                        <p className="text-xs font-bold text-gray-600">Selecione <span className="font-black text-slate-800">"Abrir no navegador"</span>, <span className="font-black text-slate-800">"Abrir no Chrome"</span> ou <span className="font-black text-slate-800">"Abrir no Safari"</span>.</p>
                      </div>
                      <div className="flex items-start gap-3">
                        <div className="w-5 h-5 rounded-full bg-slate-900 text-white text-[10px] font-black flex items-center justify-center shrink-0">3</div>
                        <p className="text-xs font-bold text-gray-600">Ao abrir no navegador oficial, o botão de instalação funcionará perfeitamente!</p>
                      </div>
                    </div>
                  </div>
                </div>
              ) : isIOS ? (
                // Scenario 2: iOS/Safari Device
                <div className="space-y-4">
                  <p className="text-xs font-bold text-gray-600 leading-relaxed">
                    No iPhone (iOS), a Apple requer que você adicione manualmente à tela de início usando o navegador Safari:
                  </p>
                  
                  <div className="space-y-3">
                    <div className="flex items-start gap-3 text-left">
                      <div className="w-6 h-6 rounded-full bg-slate-900 text-white text-[11px] font-black flex items-center justify-center shrink-0">1</div>
                      <p className="text-xs font-bold text-gray-700">Garanta que está usando o navegador <span className="font-extrabold text-blue-600">Safari</span>.</p>
                    </div>
                    <div className="flex items-start gap-3 text-left">
                      <div className="w-6 h-6 rounded-full bg-slate-900 text-white text-[11px] font-black flex items-center justify-center shrink-0">2</div>
                      <p className="text-xs font-bold text-gray-700">Toque no ícone de <span className="font-extrabold text-slate-800">Compartilhar</span> (ícone quadrado com seta para cima na barra inferior).</p>
                    </div>
                    <div className="flex items-start gap-3 text-left">
                      <div className="w-6 h-6 rounded-full bg-slate-900 text-white text-[11px] font-black flex items-center justify-center shrink-0">3</div>
                      <p className="text-xs font-bold text-gray-700">Role a lista para baixo e toque em <span className="font-extrabold text-amber-600">"Adicionar à Tela de Início"</span>.</p>
                    </div>
                  </div>

                  <p className="text-[10px] text-gray-400 font-bold uppercase tracking-wider text-left bg-gray-50 p-3 rounded-xl border border-gray-100">
                    💡 Pronto! Ele será instalado como aplicativo oficial, iniciando em tela cheia com alta performance.
                  </p>
                </div>
              ) : isInstallable ? (
                // Scenario 3: Android with Chrome (Install Prompt Active!)
                <div className="space-y-5 text-center">
                  <p className="text-xs font-bold text-gray-600 leading-relaxed text-left">
                    Seu celular é totalmente compatível! Clique no botão abaixo para transformar o cardápio em um aplicativo no seu aparelho:
                  </p>
                  
                  <button
                    onClick={() => {
                      handleInstallPWA();
                      setShowPwaAssistant(false);
                    }}
                    className="w-full py-3.5 bg-emerald-600 hover:bg-emerald-500 text-white font-black rounded-2xl shadow-md transition-all uppercase tracking-wider text-xs flex items-center justify-center gap-2"
                  >
                    <Smartphone className="w-4 h-4 animate-bounce" /> Instalar Aplicativo Agora
                  </button>

                  <p className="text-[10px] text-gray-400 font-bold uppercase tracking-wider text-left bg-gray-50 p-3 rounded-xl border border-gray-100 leading-normal">
                    💡 Rápido e prático: não ocupa memória, não precisa de loja de aplicativos e você acessa com um toque.
                  </p>
                </div>
              ) : (
                // Scenario 4: Android / PC but prompt not ready
                <div className="space-y-4">
                  <p className="text-xs font-bold text-gray-600 leading-relaxed">
                    Siga o passo a passo simples para instalar no seu navegador Android:
                  </p>

                  <div className="space-y-3">
                    <div className="flex items-start gap-3 text-left">
                      <div className="w-6 h-6 rounded-full bg-slate-900 text-white text-[11px] font-black flex items-center justify-center shrink-0">1</div>
                      <p className="text-xs font-bold text-gray-700">Toque nos <span className="font-extrabold text-slate-800">três pontinhos (⋮)</span> no canto superior direito do seu navegador.</p>
                    </div>
                    <div className="flex items-start gap-3 text-left">
                      <div className="w-6 h-6 rounded-full bg-slate-900 text-white text-[11px] font-black flex items-center justify-center shrink-0">2</div>
                      <p className="text-xs font-bold text-gray-700">Procure e toque em <span className="font-extrabold text-amber-600">"Instalar aplicativo"</span> ou <span className="font-extrabold text-slate-800">"Adicionar à tela inicial"</span>.</p>
                    </div>
                  </div>

                  <p className="text-[10px] text-gray-400 font-bold uppercase tracking-wider text-left bg-gray-50 p-3 rounded-xl border border-gray-100">
                    💡 Feito! Agora o cardápio digital terá um ícone próprio e funcionará como um app nativo.
                  </p>
                </div>
              )}
            </motion.div>
